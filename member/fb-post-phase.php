...
if($run){
    ...
    while($get_all_users = $stmt_all_users->fetch_object()){
        ...
        if($access_post){
            ...
            $stmt_history_users = $mysqli->query("select * from getrich_contents_post where post_user = ".$user_id." and post_status='success' and note LIKE '%facebook%' and note LIKE '%Phase".$_GET['phase']."%' order by post_id DESC LIMIT 0,1");
            ...
            $stmt_schedule_users = $mysqli->query("select * from getrich_schedule_fb where schedule_user=".$user_id." and schedule_phase=".$_GET['phase']);
            if($stmt_schedule_users->num_rows > 0){
                ...
                $get_facebook = getDataByCondition("getrich_users_facebook","facebook_status = 1 and facebook_user = ".$user_id,"facebook_type ASC");
                if($get_facebook->num_rows > 0){
                    $acc = 1;
                    while($user_facebook = $get_facebook->fetch_object()){
                        echo "Account ".$acc." : ".$user_facebook->facebook_name."<br>";
                        if($get_content_users->content_type == "text"){
                            $post = fbPostText($user_facebook->facebook_token,$user_id,$get_content_users->content_desc);
                            $note = $post.". | Phase".$_GET['phase']." | ".$user_facebook->facebook_name;
                        }else if($get_content_users->content_type == "photo"){
                            $post = fbPostPhoto($user_facebook->facebook_token,$user_id,$get_content_users->content_desc,$get_content_users->content_url);
                            $note = $post.". | Phase".$_GET['phase']." | ".$user_facebook->facebook_name;
                        }else if($get_content_users->content_type == "video"){
                            $post = fbPostVideo($user_facebook->facebook_token,$user_id,$get_content_users->content_title,$get_content_users->content_desc,$get_content_users->content_url);
                            $note = $post.". | Phase".$_GET['phase']." | ".$user_facebook->facebook_name;
                        }else{
                    ...
                    $note = "No one facebook account linked";
                    $sql = "insert into getrich_contents_post(post_user,post_content,post_status,note,created) values(".$user_id.",'".$schedule_content."','".$schedule_status."','".$note."','".$nowFormat."')";
                    $mysqli->query($sql);
                }
            }else{
                ...
                $note = "No one facebook content listed in phase ".$_GET['phase'];
                $sql = "insert into getrich_contents_post(post_user,post_content,post_status,note,created) values(".$user_id.",'".$schedule_content."','".$schedule_status."','".$note."','".$nowFormat."')";
                $mysqli->query($sql);
            }
        }else{
